# -*- coding: utf-8 -*-
"""
Created on Mon May 23 15:32:46 2022
    ...
    def SelectAction(self,actions,pState):
        ...
        if self.round < 6:
            ...
            if random.uniform(0, 1) <= episodes:
                ...
                QValue = self.CalQValue(deepcopy(pState), bestAction, actions)
                bestQValue = QValue
            ...
            for nAction in nextAction:
                if time.time() - startTime > THINKTIME:
                    ...
                    QValue = self.CalQValue(deepcopy(nextState), nAction, actions)
                    bestNextQ = max(bestNextQ,QValue)
                    
            features = self.DefineFeatures(deepcopy(pState), bestAction, actions)
